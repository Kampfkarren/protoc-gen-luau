...
fn add_message_descriptors(
	...
	for nested_enum in &descriptor.enum_type {
		add_enum_descriptors(nested_enum, export_map, &format!("{package}.{}", descriptor.name()), path, &format!("{prefix}{}_", descriptor.name()));
	}
}
...
fn create_decoder(fields: BTreeMap<i32, String>) -> String {
	...
	for (index, (field, code)) in fields.iter().enumerate() {
		lines.push(format!("{} field == {field} then", if index == 0 { "if" } else { "elseif" }));
...
impl<'a> FileGenerator<'a> {
	fn new(file_descriptor_proto: FileDescriptorProto, export_map: &'a ExportMap) -> FileGenerator<'a> {
		Self {
	...
	fn generate_message(
		...
		self.types.push(format!("local {name}: proto.Message<{name}>"));
	...
	fn generate_enum(&mut self, descriptor: &EnumDescriptorProto, prefix: &str) {
		...
		for (index, field) in descriptor.value.iter().enumerate() {
			self.types.push(format!("{}\"{}\"", if index == 0 { "" } else { "| " }, field.name()));
		...
		self.implementations.push(
			ENUM
				.replace("<name>", &name)
				.replace("<from_number>", from_number.with_else(|builder| {
					builder.push("return nil");
				}).build().trim_start())
				.replace("<to_number>", to_number.with_else(|builder| {
